...
팀이 확정되면 봇이 음성 채널을 준비하고, 합의가 끝난 뒤 버튼으로 이동할 수 있습니다:

1. "내전" 카테고리 생성 (없으면)
2. "🔵 블루팀", "🔴 레드팀" 음성 채널 생성 (없으면)
3. 팀 확인 후 `이동하기` 버튼을 누르면 각 팀원을 해당 음성 채널로 이동

> ⚠️ 참가자가 아무 음성 채널에도 접속해 있지 않으면 이동이 불가합니다. 이동 버튼을 누르기 전에 음성 채널에 접속해주세요.
...
#### 라인별 밸런스 가중치 상향 (v1.5.2)

- 팀 매칭 시 라인별 점수 차이 가중치를 2 → 3으로 상향했습니다.
- 같은 라인끼리 실력이 더 비슷하게 매칭됩니다.

### 2026-03-18

#### 팀 라인 표시 순서 정렬 (v1.5.1)

- `!내전` 팀 결과가 탑, 정글, 미드, 원딜, 서폿 순서로 고정 표시됩니다.
- `!팀테스트` 결과도 동일한 라인 순서로 표시됩니다.

#### 내전 분석 속도 개선 & 수동 팀 이동 (v1.5.0)

- `!내전` 전적 분석 속도가 개선되어 팀 배정 대기 시간이 줄었습니다.
- `!내전` 전적 분석 중 현재 진행 상황이 표시됩니다.
- 팀 결과가 나온 뒤 자동 이동 대신 `이동하기` 버튼으로 합의 후 음성 채널 이동할 수 있습니다.

#### 라인 자동 분석 강화 & 라인 조회 (v1.4.0)

- 내전 시작 시 라인 선호도 미등록 유저는 전적 기반으로 1순위/2순위 라인이 자동 적용됩니다.
- 외부 참가자(`!참가인원추가`)도 동일하게 전적 기반 라인 자동 배정됩니다.
- `!라인` — 인자 없이 입력하면 현재 설정된 라인을 조회할 수 있습니다.

#### 연동 시 선호 라인 자동 설정 & 패치노트 조회 (v1.3.0)

- `!연동` 시 최근 전적 기반으로 선호 라인이 자동 설정됩니다. (이미 `!라인`으로 등록한 유저는 영향 없음)
- `!패치노트` — 최신 패치노트 조회, `!패치노트 1.1.0` 으로 특정 버전도 조회 가능
- `!도움말`에 `!help` 별칭 추가

#### 패치노트 자동 공지 기능 (v1.2.0)

| 명령어 | 설명 |
| --- | --- |
| `!공지채널` | 현재 채널을 패치노트 공지 채널로 설정 (서버 관리 권한 필요) |

- 봇이 업데이트되면 설정된 채널에 패치노트가 자동으로 전송됩니다.
- 같은 버전은 중복 공지되지 않습니다.
