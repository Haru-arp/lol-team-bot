...
| `!전적 [소환사명#태그]`   | 전적 조회, 인자 없으면 대표 계정 조회            |
...
```
score = 티어 점수 × 0.4 + 승률 × 0.25 + 주 라인 숙련도 × 0.2 + 챔피언 풀 × 0.15
```

- **티어 점수** — IRON(1) ~ CHALLENGER(10), 디비전별 +0~0.75
- **승률** — 최근 20판 기준
...
```
총점 = (팀 총점 차이 × 2) + (라인별 점수 차이 합 × 1) + (성향 불균형 패널티)
```

- **팀 총점 차이** — 블루팀 합산 vs 레드팀 합산 (가중치 2, 가장 중요)
- **라인별 차이** — 탑 vs 탑, 미드 vs 미드 등 같은 라인끼리 점수 차이
