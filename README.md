...
- `!티어세팅 소환사명#태그 골드2` 형태로 다른 사람 계정에도 지정할 수 있으며, 이 경우 서버 관리 권한이 필요합니다.
...
연동하지 않은 유저도 내전에 참가시킬 수 있습니다.

| 명령어 | 설명 |
| --- | --- |
| `!참가인원추가 @유저 소환사명#태그` | 디스코드 멘션 + Riot ID로 추가 (음성 이동 ✅) |
| `!참가인원추가 소환사명#태그` | Riot ID만으로 추가 (음성 이동 ❌) |
| `!참가인원제거 @유저` | 멘션으로 참가자 제거 |
| `!참가인원제거 소환사명#태그` | Riot ID로 참가자 제거 |

- 외부 참가자도 Riot API 기반 전적 분석, 팀 매칭, 밴 추천이 동일하게 적용됩니다.
- 라인 선호도는 Riot 전적 기반 주 라인으로 자동 배정됩니다.
- 디스코드 멘션 없이 추가된 참가자(`ext_` ID)는 내전 승패 랭킹에 기록되지 않습니다.
